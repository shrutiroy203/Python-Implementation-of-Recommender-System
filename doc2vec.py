...
def load_glove(filename):
    ...
    with open(filename,'r') as f:
        for line in f:
            d[line.split(' ')[0]]=np.array(line.split(' ')[1:],dtype=float)
...
def words(text):
    ...
    """
    str_without_punc=re.sub('['+string.punctuation+'0-9\\r\\t\\n]', '', text)
    regex = re.compile('\s+')
    str_split=regex.split(str_without_punc)
    str_final=[x.lower() for x in str_split if(x.lower() not in ENGLISH_STOP_WORDS and len(x)>=3)]
...
def load_articles(articles_dirname, gloves):
    ...
    for f in files:
        ...
        fname=''.join(head.split('/')[-1:]+["/"]+tail.split())
        result.append((fname,title,body,centroid))
...
def doc2vec(text, gloves):
    ...
    for w in extracted_words:
        if w in gloves:
            n+=1
            if(len(centroid)==0):
                centroid=gloves[w].copy()
            else:
                centroid+=gloves[w] # temp variable holding vector of the word
...
def distances(article, articles):
    ...
    for a in articles:
        if a!=article:
            result.append((np.linalg.norm(article[3]-a[3]),a))
...
def recommended(article, articles, n):
    ...
    rec_articles=distance_list[:n]
    result=[x[1] for x in rec_articles]
